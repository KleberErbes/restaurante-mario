...
document.addEventListener('DOMContentLoaded', () => {
  ...
  adminVerificarURL(); // Detecta #admin na URL
});
// ========== PAINEL ADMIN ==========
...
// Detecta #admin na URL ao carregar
function adminVerificarURL() {
  if (window.location.hash === '#admin') {
    adminAbrirSenha();
    // Remove o hash da URL para não aparecer na barra do navegador
...
// Carrega as configs da planilha
async function adminCarregarConfigs() {
  try {
    ...
    // Preenche os campos com os valores da planilha
    document.getElementById('cfgHorarioPedidos').value   = cfg.horarioPedidos   || '08:00';
    document.getElementById('cfgHorarioAbertura').value  = cfg.horarioAbertura  || '11:00';
    document.getElementById('cfgHorarioFechamento').value = cfg.horarioFechamento || '14:00';
    ...
    document.getElementById('cfgDiasFechados').value       = cfg.diasFechados       || '';

    document.getElementById('adminLoadingMsg').style.display = 'none';
    document.getElementById('adminContent').style.display = 'block';
  } catch (e) {
    // Se falhar, carrega com os valores atuais do código
    document.getElementById('cfgHorarioPedidos').value   = `${String(HORARIO_PEDIDOS.h).padStart(2,'0')}:${String(HORARIO_PEDIDOS.m).padStart(2,'0')}`;
    document.getElementById('cfgHorarioAbertura').value  = `${String(HORARIO_ABERTURA.h).padStart(2,'0')}:${String(HORARIO_ABERTURA.m).padStart(2,'0')}`;
    document.getElementById('cfgHorarioFechamento').value = `${String(HORARIO_FECHAMENTO.h).padStart(2,'0')}:${String(HORARIO_FECHAMENTO.m).padStart(2,'0')}`;
    document.getElementById('cfgPrecoMedia').value         = '26.00';
    document.getElementById('cfgPrecoGrande').value        = '28.00';
    document.getElementById('cfgPrecoBuffetSemana').value  = '39.00';
    document.getElementById('cfgPrecoBuffetSabado').value  = '42.00';
    document.getElementById('cfgDiasFechados').value       = DIAS_FECHADOS_ESPECIAIS.join('\n');
    document.getElementById('adminLoadingMsg').style.display = 'none';
    document.getElementById('adminContent').style.display = 'block';
    showToast('⚠️ Não foi possível carregar configs da planilha. Usando valores locais.', 'aviso', 5000);
  }
}
...
async function adminSalvar() {
  ...
  try {
    ...
    adminAplicarConfigs({ horarioPedidos, horarioAbertura, horarioFechamento, precoMedia, precoGrande, precoBuffetSemana, precoBuffetSabado, diasFechados });
...
function adminAplicarConfigs(cfg) {
  ...
  const txtPedidos = fmtH(ph, pm) + " até as " + fmtH(fh, fm);
  ...
  showToast('✅ Configurações aplicadas!', 'sucesso', 3000);
}
